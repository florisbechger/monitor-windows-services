...
.DESCRIPTION
    This script is a Controller Script to trigger a specified action on a Windows Server for Quintiq Services to Test/Start/Stop Services. The File-name must be specified as "qservices_[test/up/down].ps1"
...
## This loop will START all selected Services ##
$UP = {$start = Get-Service -ComputerName "$Server" | Where-Object {$_.name -eq "$Service"}
Try {
	if ($start.status -eq "Stopped") {$start.Start()}
	do {
	$count1 = (Get-Service -ComputerName "$Server" | Where-Object {$_.name -eq "$Service"} | Where-Object {$_.status -eq "Stopped"}).count
	$maxRepeat--
	Start-Sleep -Milliseconds 600
	$count2 = (Get-Service -ComputerName "$Server" | Where-Object {$_.name -eq "$Service"} | Where-Object {$_.status -eq "StartPending"}).count
	$maxRepeat--
	Start-Sleep -Milliseconds 600
	$count3 = (Get-Service -ComputerName "$Server" | Where-Object {$_.name -eq "$Service"} | Where-Object {$_.status -eq "Running"}).count
	$maxRepeat--
	Start-Sleep -Milliseconds 600
	$count4 = (Get-Service -ComputerName "$Server" | Where-Object {$_.name -eq "$Service"} | Where-Object {$_.StartType -eq "Disabled"}).count
	$maxRepeat--
	Start-Sleep -Milliseconds 600 -ErrorAction Stop
	} until ($count1 -like 0 -and $count2 -like 0 -and $count3 -like 1 -or $count4 -like 1 -or $maxRepeat -eq 0)
	Start-Sleep -s $Wait
	}
Catch {
	Write-Warning "   $Service is disabled"
}}

## Start logging ##
Start-Transcript -Path $Admin\$DesktopFolder\$ScriptFolder\$LogFolder\$Server-$Date-up.log -Append
...
## Running State ##
$State = $UP
Write-Host "`n2. UP State selected, this will result in Services started:"
